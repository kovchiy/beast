...
Элементами, как правило, назначаются компоненты, которые вынуждены часто общаться с себе подобными и зависят от какого-то общего параметра (модификатора родительского блока, например). Им просто удобно быть рядом и образовывать изолированную систему частого обмена сообщениями. Границы такой группы обозначает родительский компонент — блок; он же хранит общие знаменатели группы.
...
Несмотря на то, что HTML генерируется автоматически, логику генерации все равно следует понимать, чтобы писать CSS и селекторы для деклараций (о последних речь идет в следующей части). Итак, в HTML вся семантика из названий узлов ушла в их классы. Названия тоже частично поменялись: вместо `Browser` теперь `div class="browser"`, а вместо `tabs` `div class ="browser__tabs"`. Сделано это для того, чтобы каждый блок или элемент с модификатором или без можно было описать одним CSS-селектором:
...
Такая нотация, во-первых, ускоряет время рендера стилей в браузере за счет сокращения количества селекторов для идентификации компонента и его модификации, а, во-вторых, страхует от пересечений селекторов стилей при вложении одних компонент в другие. С использованием препроцессоров (например, Less) классы перестают пугать своей длиной:
...
Работа с компонентами ведется внутри деклараций. Соответствие компонента и декларации устанавливается через селектор. Декларации исполняются в момент вызова метода `render()`. Метод `render` вызывается автоматически при создании корневого компонента внутри тега `<script type="bml">`:
...
Метод `mod` возвращает значение модификатора. В данном случае, если нет модификатора `no-title`, можно выводить заголовок. Метод `empty` очищает содержимое компонента перед началом присоединения дочерних элементов. Вне зависимости от порядка входных данных, имя автора должно стоять в конце статьи.
...
##### Поля mod и param
Также на этапе развертывания можно определить модификаторы и параметры компонента с их значениями по умолчанию. Такие определения полезны для авто спецификации API.
...
Последовательность обхода: Browser, head, Tabs, Addressbar, content, Webview. Сделано это для того, чтобы всегда оставался шанс до уточнить содержимое и развернуть его тоже:
...
Названия событий общей шины от компонент автоматически предваряются селектором (родительского в случае с элементом) блока, чтобы исключить пересечение имен и сделать подписку более наглядной. В примере `popup` подписывается на событие открытия себе подобных и закрывается.
...
Получить массив элементов блока без учета уровня вложенности. Метод актуален не только для блока. Крайне не рекомендуется использовать этот метод в паре с `append` — для этого подойдет метод `get()`, вынимающий только ближайших детей.
...
Функция `callback` выполнится только после DOM-инициализации компонента. Полезно, когда элементу требуется взаимодействовать с инициализированным родительским блоком (а элементы инициализируются вперед).
...
Повторная перерисовка блока с новыми входными данными, с разметкой и инициализацией. Текущие дети блока удалятся, но модификаторы и параметры останутся. 
...
Практические советы, механизмы оптимальной организации сложных связей между компонентами, принятый codestyle, разработка проекта с чистого листа — всё это описывает учебник [Beast Practice](/docs/beast-practice/README.md). 
